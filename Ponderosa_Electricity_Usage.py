...
def main():
   ...
   while True:
      ...
      while (checkResponse):
         ...
         if (response is None):
            print("==> Call to api.get_instantaneous_demand() failed! #" + str(i))
            checkResponse = True
         else:
            checkResponse = False
            if (i == 5):
      ...
      if (checkResponse):
         ...
         if (os.path.exists(alreadyRunning)):
            os.remove(alreadyRunning)
         mytime = time.localtime()
         print("==> api.get_instantaneous_demand() failed  at " + time.strftime('%m/%d/%Y %H:%M:%S', mytime))
         sys.exit(2)
      ...
      if (the_hour == the_hour_last):  # Still in same hour
         ...
         if (minute_counter > 0):  # NOT time through loop
            kWh = decimal.Decimal(minute_sumkw / minute_counter)
            f.write(the_date_last + "\t" + str(the_hour_last) + ":00\t" + str(kWh) + "\n")
            f.flush()
      ...
      if (the_hour != 0):  # Not Midnight; set flag for next Midnight
         printMidnight = True
      elif (the_hour == 0 and printMidnight):  # Midnight; print summary only once
         ...
         sys.stdout.flush()
         sys.stderr.flush()
         printMidnight = False  # Reset flag
         kWh_day = 0  # Reset Sum
